...
namespace TeamComposition2.GameModes.Physics
{
    ...
    {
        private static readonly List<GameObject> ItemsToRemoveOnPointEnd = new List<GameObject>();
    ...
    public abstract class PhysicsItem : MonoBehaviour
    {
        ...
        public static readonly int ColliderLayer = LayerMask.NameToLayer("IgnorePlayer");
    ...
    public abstract class NetworkPhysicsItem<TCollider, TTrigger> : PhysicsItem, IPunInstantiateMagicCallback, IPunObservable where TCollider : Collider2D where TTrigger : Collider2D
    {
        ...
        protected object GetSyncedData(string key, object defaultValue = default)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"Key \"{key}\" not found in syncing data of NetworkPhysicsItem component of {this.name}.");
            }
            return defaultValue;
        }
        protected int GetSyncedInt(string key, int defaultValue = default)
        {
            if (this._intDataToSync.TryGetValue(key, out int value))
            {
                return value;
        ...
        protected string GetSyncedString(string key, string defaultValue = default)
        {
        ...
        protected float GetSyncedFloat(string key, float defaultValue = default)
        {
        ...
        private readonly List<ItemSyncPackage> syncPackages = new List<ItemSyncPackage>();
        ...
        protected virtual void Start()
        {
            ...
            if (this.View != null && this.View.ObservedComponents != null)
            {
                this.View.ObservedComponents.Add(this);
            }
        }
        ...
        {
            this.Rig.drag = Mathf.LerpUnclamped(this.PhysicalProperties.MinDrag, this.PhysicalProperties.MaxDrag, this.Rig.velocity.sqrMagnitude / this.PhysicalProperties.MaxSpeedSqr);
            this.Rig.angularDrag = Mathf.LerpUnclamped(this.PhysicalProperties.MinAngularDrag, this.PhysicalProperties.MaxAngularDrag, Mathf.Abs(this.Rig.angularVelocity) / this.PhysicalProperties.MaxAngularSpeed);
        }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            else
            {
                ...
                itemSyncPackage.timeDelta = Mathf.Clamp((float)stream.ReceiveNext(), 0f, 0.1f);
